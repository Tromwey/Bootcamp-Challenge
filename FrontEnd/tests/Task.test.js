...
fixture ('Task - Tasks Creation feature test')
    .page `${URLS.BASE_URL}`
    .beforeEach( async t => {
...
test.meta({ feature: 'task', type: 'smoke' })('As a user, I should be able to create a task by providig a name', async () => {
    await todayPage.createTasks()
    await inboxPage.assertCreatedTasks(TASK.DEFAULT_NAME, TASK.TODAY, TASK.SINGLE_TASK)
})

test.meta({ feature: 'task', type: 'smoke' })('As a user, I should be able to create a task selecting tomorow as due date', async () => {
...
test.meta({ feature: 'task' })('As a user, I should be able to create multiple tasks', async () => {
